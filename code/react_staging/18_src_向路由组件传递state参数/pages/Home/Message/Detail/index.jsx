...
export default class Detail extends Component {
	render() {
		...
		//虽然我们是把数据传入到location的state里的，但其实location是在history里的，是history的属性(最开始介绍三大路由属性里提到过)。
		//但如果清除了浏览器的缓存，重新输入这个地址，会报错，提示this.props.location.state是undefined
		//所以无法执行const {id,title} = this.props.location.state这段代码
		//解决方式：我们可以在后面加上 || {} 意思是：或空对象
		// const {id,title} = this.props.location.state || {}
		//如果有值，我就用；没有值，我就用空对象。这样不会报错，顶多id和title是undefined,页面上不显示内容，但不会报错。
		//(补充：上面之所以报错，是因为this.props.location.state是undefined，因此不能从undefined上取数据。但{}可以取数据，只不过取到的id和title是undefined。且state默认是undefined，等着你传递对象呢)
