//Count的容器组件：
//容器组件的创建不能自己写，它是一个连接redux和UI组件的桥梁。需要借助react-redux的connect，下详
//yarn add react-redux
//容器组件的作用就是连接redux和UI组件
...
//connect()的返回值也是一个函数，并在这里继续调用，形参是Count的UI组件
...
//函数名是官方建议的，虽然你可以叫别的，但一般还是用mapStateToProps和mapDispatchToProps
//补充：我曾经设想能不能在mapStateToProps里传入函数而不是保存状态的对象(把两个函数的作用互换)，但官方做了判断，必须传入对象，同理mapDispatchToProps也一样。
//但问题又来了，mapDispatchToProps里确实是对象，但对象里是一个一个的函数，我能不能对mapStateToProps也这样做呢？有待测试。

/* 函数1： 该函数的返回值会作为状态传递给UI组件：
	1.mapStateToProps函数返回的是一个对象；
	2.返回的对象中的key就作为传递给UI组件props的key,value就作为传递给UI组件props的value
	3.mapStateToProps用于传递状态
	4.函数名是官方建议的，虽然你可以叫别的，但一般还是用mapStateToProps和mapDispatchToProps

	因为props是一组一组的key-value，不能能光传递value，因为在取value的时候需要通过key去取value
	所以很自然的需要return一个对象
*/
//用箭头函数也行
function mapStateToProps(state) {//state就是redux里帮你保存的状态，此案例里是0，在count_reducer.js里定义的
...
function mapDispatchToProps(dispatch) {
	return {
		//子组件只要调用jia这个方法，这里就会收到number，然后通知redux通过dispatch进行运算。
