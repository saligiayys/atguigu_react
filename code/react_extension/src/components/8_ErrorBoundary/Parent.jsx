...
export default class Parent extends Component {
	//错误边界
	//我们需要使用一个类似getDerivedStateFromProps生命周期钩子的钩子getDerivedStateFromError()，需要时静态的
	//意思是，那这个案例举例：如果Parent的子组件Child出现了任何形式的报错，都会调用这个钩子，会把错误作为形参传入
	...
	// 只能捕获后代组件生命周期产生的错误，比如render, componentDidMount等，不能捕获自己组件产生的错误和其他组件在合成事件、定时器中产生的错误
	// 也就是如果是一个自定义demo()的函数出错了，不会捕获。
	//其实，我们一般想捕获的就是render里的错误

	//但是错误边界只适用于生产环境，不能用于开发环境。
	//所以在这里你通过WebStorm或VsCode开启后会发现，它会先提示错误，然后马上整个应用又报错了，因为我们是开发人员，不是用户，我们要解决问题
	...
	static getDerivedStateFromError(error) {//名称直译：从错误获取一个衍生的状态
		console.log('@@@', error);
		return { hasError: error }//返回一个状态对象，更新状态，不用写setState，因为名称直译：从错误获取一个衍生的状态。
	...
	//弹幕：戏称加班函数
	componentDidCatch() {
		console.log('此处用于统计错误，反馈给服务器，用于通知编码人员进行bug的解决。也就是错误日志');
		//统计错误，反馈给服务器的代码。。。
	}
